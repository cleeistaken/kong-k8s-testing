...
1. In WCP, create a name space names '**kong**' 
2. Add '**vsan-esa-default-policy-raid5**' storage policy to '**kong**' namespace
3. Create and add a VM class named '**vks-8-32-class**' and add it to '**kong**' namespace
   * This class will be used for the VKS worker nodes
   * Define the 8 vCPU
   * Define 32GB of RAM. 
4. Add '**best-effort-medium**' VM class to '**kong**' namespace
...
SUPERVISOR_NAMESPACE_NAME="kong"
SUPERVISOR_CONTEXT="kong-ctx"
CLUSTER_NAME="kong-vks"
CLUSTER_NAMESPACE_NAME="kong-ns"
```
...
```shell
# Create a context named 'kong-ctx'
...
### 4. Create 'kong-vks' VKS cluster
```shell
# Create a VKS cluster as defined in vks.yaml
kubectl apply -f vks.yaml

# Expected output:
# calicoconfig.cni.tanzu.vmware.com/kong-vks created
# clusterbootstrap.run.tanzu.vmware.com/kong-vks created
...
# 
# or:
# calicoconfig.cni.tanzu.vmware.com/kong-vks unchanged
# clusterbootstrap.run.tanzu.vmware.com/kong-vks unchanged
# Warning: cluster.x-k8s.io/v1beta1 Cluster is deprecated; use cluster.x-k8s.io/v1beta2 Cluster
# cluster.cluster.x-k8s.io/kong-vks created
...
### 5. Connect to 'kong-vks' VKS cluster
```shell
...
# antrea-agent  5         5         5       5            5           kubernetes.io/os=linux   38m
# kube-proxy    5         5         5       5            5           kubernetes.io/os=linux   38m
```

### 6. Create 'kong-ns' namespace
```shell
...
#
# or:
# Error from server (AlreadyExists): namespaces "kong-ns" already exists

# Set context
kubectl config set-context --current --namespace=$CLUSTER_NAMESPACE_NAME

# Expected output:
# Context "vks:kong-vks" modified.
